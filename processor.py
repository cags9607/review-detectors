...
from core import WIDE_OUTPUT_COLS, normalize_output_format, predict_records
from processor_config import (
    AI_REVIEW_BATCH_SIZE,
    AI_REVIEW_DETECTOR_NAME,
...
# The queue/ClickHouse long-format contract is intentionally minimal.
# Core/local inference may retain richer diagnostic fields; queue pushes do not.
QUEUE_LONG_OUTPUT_COLS = [
    "store",
    "bundle_id",
    "review_id",
    "detector",
...
def _set_model_env():
    ...
    os.environ["SCAM_REVIEW_DETECTOR_NAME"] = SCAM_REVIEW_DETECTOR_NAME
    ...
    os.environ["AI_REVIEW_DETECTOR_NAME"] = AI_REVIEW_DETECTOR_NAME
...
def _clean_prediction_result(prediction: Dict[str, Any], output_format: str = "long") -> Dict[str, Any]:
    ...
    return {
        col: _json_safe_value(prediction.get(col))
        for col in QUEUE_OUTPUT_COLS_BY_FORMAT[output_format]
    }
...
def run_once() -> int:
    ...
    processed_jobs = process_jobs(jobs, output_format = REVIEW_DETECTORS_OUTPUT_FORMAT)
...
def main():
    ...
    logger.info(
        "Starting combined review detectors processor: batch_size=%s, output_format=%s, scam_detector=%s, ai_detector=%s, scam_hf=%s/%s, ai_hf=%s, ai_min_words=%s.",
        BATCH_SIZE,
        REVIEW_DETECTORS_OUTPUT_FORMAT,
        SCAM_REVIEW_DETECTOR_NAME,
        AI_REVIEW_DETECTOR_NAME,
