...
def _clean_prediction_result(prediction: Dict[str, Any]) -> Dict[str, Any]:
    return {col: prediction.get(col) for col in QUEUE_OUTPUT_COLS}
...
def process_jobs(jobs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if len(predictions) != len(records):
        raise RuntimeError(
            f"Prediction count mismatch: got {len(predictions)} predictions for {len(records)} records."
        )
...
def main():
    ...
    logger.info(
        "Starting combined review detectors processor: batch_size=%s, scam_hf=%s/%s, ai_hf=%s, ai_min_words=%s.",
        BATCH_SIZE,
        SCAM_REVIEW_HF_REPO_ID,
        SCAM_REVIEW_SUBFOLDER,
        AI_REVIEW_HF_REPO_ID,
