...
The scam detector uses `DeepSee-io/app-high-risk-signals-classifiers` with the default subfolder `scam_reviews`.
The AI-review detector uses `DeepSee-io/qwen_adapters_aigt` with a configurable language-to-checkpoint-root mapping. For the reviews model, the configured root is `reviews/best`; the AI loader resolves its nested PEFT files under `reviews/best/lora_adapter/` and its `head.pt`/`train_config.json` from `reviews/best/`. Do not configure the mapping as `reviews/best/lora_adapter`.
...
```bash
SCAM_REVIEW_HF_REPO_ID=DeepSee-io/app-high-risk-signals-classifiers
SCAM_REVIEW_SUBFOLDER=scam_reviews
SCAM_REVIEW_DETECTOR_NAME=scam
AI_REVIEW_HF_REPO_ID=DeepSee-io/qwen_adapters_aigt
AI_REVIEW_SUBDIR_BY_LANG_JSON='{"en":"reviews/best"}'  # checkpoint root; loader appends /lora_adapter for PEFT files
...
### AI Hugging Face asset layout

The two detectors do not store adapters with the same directory layout.

```text
Scam: DeepSee-io/app-high-risk-signals-classifiers/
└── reviews_scam/
    └── adapter_config.json

AI: DeepSee-io/qwen_adapters_aigt/
└── reviews/
    └── best/                         # value used in AI_REVIEW_SUBDIR_BY_LANG_JSON
...
Accordingly, use `{"en":"reviews/best"}` for the AI mapping. The AI model loader adds `lora_adapter/` internally when it loads the PEFT adapter.
