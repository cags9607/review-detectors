...
def predict_records(
    ...
    Returns:
        ...
        a ``scam_review`` row and an ``ai_review`` row. For reviews that are too
        short or otherwise not scorable by the AI detector, the ``ai_review``
        row is retained with null label/confidence/score fields.
    """
